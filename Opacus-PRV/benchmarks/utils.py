...
def get_layer_set(layer: str) -> str:
    ...
    """
    layer_set = layer.replace("gsm_dp", "").replace("gsm_",
                                                    "").replace("dp", "")
...
def generate_report(path_to_results: str, save_path: str, format: str) -> None:
    ...
    path_to_results = (path_to_results
                       if path_to_results[-1] != "/" else path_to_results[:-1])
    files = glob.glob(f"{path_to_results}/*")

    if len(files) == 0:
        raise Exception(
            f"There were no result files in the path {path_to_results}")
    ...
    for raw in raw_results:
        ...
        memory = np.mean([
            i["memory_stats"]["max_memory"] for i in raw["results"]
    ...
    pivot = results.pivot_table(
        index=[
            "batch_size", "num_runs", "num_repeats", "forward_only", "layer"
        ],
        columns=["gsm_mode"],
    ...
    def add_ratio(df, metric, variant):
        ...
        df[(metric, f"{variant}/baseline")] = (df.loc[:, (metric, variant)] /
                                               df.loc[:, (metric, "baseline")])
