...
class PRVAccountant(IAccountant):
    ...
    def get_epsilon(
            ...
            *,
            ...
            sample_rate=None,  # 
            num_updates=None,  # used for jax code
        ...
        if isinstance(num_updates, int):
            self.history = [(noise_multiplier, sample_rate, num_updates)]
    ...
    def _get_dprv(
        ...
        # use self.PoissonSubsampledEPMPRV_f in order to not reinitalize the PRV sampler
        if beta is None:
            prvs = [
                PoissonSubsampledGaussianPRV(sample_rate=sample_rate,
                                             noise_multiplier=noise_multiplier)
            ...
            prvs = [
                PoissonSubsampledGenericPRV(
                    cdf_f=self.PoissonSubsampledEPMPRV_f,
                    sample_rate=sample_rate,
                    noise_multiplier=noise_multiplier,
                    beta=beta)
                for noise_multiplier, sample_rate, _ in self.history
            ...
            # prvs = [
            #     PoissonSubsampledEPMPRV(sample_rate=sample_rate,
            #                             noise_multiplier=noise_multiplier,
            #                             beta=beta,
            #                             mu=1.0)
            #     for noise_multiplier, sample_rate, _ in self.history
    ...
    def _get_domain(
        ...
        return Domain.create_aligned(t_min=-L, t_max=L, dt=mesh_size)
