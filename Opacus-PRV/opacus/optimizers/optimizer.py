...
class DPOptimizer(Optimizer):
    ...
    def __init__(
        ...
        if loss_reduction not in ("mean", "sum"):
            raise ValueError(
                f"Unexpected value for loss_reduction: {loss_reduction}")
        ...
        self.beta = beta
        self.beta_sampler = None

        if beta_sampler is not None:
            self.beta_sampler = beta_sampler
    ...
    def _get_flat_grad_sample(self, p: torch.Tensor):
        ...
        if isinstance(p.grad_sample, torch.Tensor):
            ...
            raise ValueError(
                f"Unexpected grad_sample type: {type(p.grad_sample)}")
    ...
    def clip_and_accumulate(self):
        ...
        if len(self.grad_samples[0]) == 0:
            ...
            per_sample_norms = torch.stack(per_param_norms, dim=1).norm(2,
                                                                        dim=1)
